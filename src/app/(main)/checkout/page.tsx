...
export default function CheckoutPage() {
  ...
  if (items.length === 0 && !placed) {
    return (
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-6 py-20 flex flex-col items-center text-center">
        ...
        <Link
          href="/products"
          className="bg-accent hover:bg-accent-hover text-white font-black uppercase text-[11px] tracking-[0.15em] px-8 py-3.5 transition-colors mt-6"
        >
          Browse Products
        </Link>
      </div>
    ...
    return (
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-6 py-16">
        <div className="max-w-lg mx-auto text-center">
          ...
          <p className="text-accent font-black text-lg mb-8">
            #{placed.orderNumber}
          </p>
          <div className="bg-[#0f0f0f] border border-[#1e1e1e] p-6 text-left mb-8 space-y-3">
            {[
              ...
              <div key={k} className="flex justify-between text-sm">
                <span className="text-[#555] font-bold uppercase tracking-wider text-[11px]">
                  {k}
                </span>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (createOrder.rejected.match(resultAction)) return;

    const order = resultAction.payload as any;

    // if khalti - intitate and redirerrct
    if (payment === "khalti") {
      try {
        const result = await initiateKhaltiPayment(order.id);
        window.location.href = result.data.payment_url; //redirect to khalti
      } catch (err: any) {
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="bg-[#0f0f0f] border border-[#1e1e1e] p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {(
                  ...
                  ] as [keyof ShippingAddress, string, string, boolean][]
                ).map(([field, label, type, req]) => (
                  <div
                    key={field}
                    className={field === "address" ? "sm:col-span-2" : ""}
                  >
                    <label className="block text-[10px] font-black uppercase tracking-[0.15em] text-[#555] mb-1.5">
                      {label}
                      {req && " *"}
                    </label>
                    <input
            ...
            <div className="bg-[#0f0f0f] border border-[#1e1e1e] p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {PAYMENT_METHODS.map((m) => (
                  <label
                    key={m.value}
                    className={`flex items-start gap-3 p-4 border cursor-pointer transition-colors ${payment === m.value ? "border-accent bg-accent/5" : "border-[#222] hover:border-[#333]"}`}
                  >
                    <input
                      type="radio"
                      name="payment"
          ...
          <div className="lg:col-span-1">
            <div className="bg-[#0f0f0f] border border-[#1e1e1e] p-5 sticky top-24">
              <h2 className="text-[11px] font-black uppercase tracking-[0.2em] text-white mb-5">
                Order Summary
              </h2>
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto mb-5 pr-1">
                {items.map((item) => (
                  <div key={item.id} className="flex gap-3 text-sm">
                    ...
                    <p className="text-accent font-bold text-xs shrink-0">
                      Rs.{" "}
              ...
              <div className="border-t border-[#1e1e1e] pt-4 space-y-2 text-sm mb-5">
                ...
                <div className="flex justify-between">
                  <span className="text-[#555]">Tax (13% VAT)</span>
